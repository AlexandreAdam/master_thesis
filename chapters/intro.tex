...
Les paramètres $\alpha$ et $\theta$ sont absorbées dans les paramètres $\varphi^{(t)}$ dans la 
relation de récurrence \eqref{eq:rim putzky}, qui nous donne 
...
sujet actif de recherche \citep[e.g.][]{}, particulièrement dans l'étude des attaques antagonistes de réseaux de neuronnes \citep[e.g.][]{}. 
Nous reportons l'étude de la troisième condition d'Hadamard pour des travaux futurs.

Finalement, on note un aspect important du modèle $g_{\varphi^{(t)}}$, soit la possible dépendance envers $t$. 

%On définit le tenseur latent $\mathbf{h}^{(t)}$ de la cellule GRU, qui joue le rôle de mémoire. On définit de plus 
%l'état de paramètres quelconques (comme l'activation interne d'une réseau de neuronnes) 
%à l'itération $t$ de la relation de récurrence $\mathbf{x}^{(t)}$. La cellule met à jour l'état interne 
%et le tenseur latent de la façon suivante
%\begin{align}
...
%\end{align}
%où $S(x) = \frac{1}{1 + e^{-x}}$ est une fonction sigmoïde. Les noyaux de convolution $\mathbf{w}$ et les vecteurs de biais $\mathbf{b}$  
