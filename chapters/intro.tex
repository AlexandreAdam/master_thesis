...
des manuels de références de \citet{Meneghetti2013} et \citet{Congdon2018}. 
...
Selon ce point de vue, $H(Z \mid X) = -\mathbb{E}_{p(\mathbf{x},\mathbf{z})}\big[ \log p(\mathbf{z} \mid \mathbf{x}) \big]$ correspond donc à une perte d'information (augmentation de notre incertitude) 
...
la fonction identité comme auto-encodeur: $f_{\phi,\theta}(X) = \bbone X$, de sortes que $\hat{X} = Z = X$. Or, $Z$ n'est pas une représentation pertinente dans ce cas. 
...
En effet, l'objectif de comprimer l'information est intimement lié à l'objectif d'obtenir un sommaire informatif; ç.-à-d. qu'un 
...
On commence par construire une limite inférieur variationnelle sur $I(Z; X)$. 
On assume d'abord que la distribution jointe $p(X, Y, Z)$ se factorise de la façon suivante
\begin{equation}\label{eq:factorisation}
       p(X, Y, Z) = p(X \mid Z) p(Y \mid X) p(X) 
\end{equation} 
où $p(X \mid Z, Y) = p(X \mid Z)$ suivant le fait qu'on assume une chaîne de Markov $Y \rightarrow X \rightarrow Z$ pour les 
variables aléatoires (voir figure \ref{fig:info VAE}).
On introduit un encodeur 
$q_\phi (\mathbf{z} \mid \mathbf{x})$ qui modélise notre système de transmission d'information par compression, $p(X \mid Z)$. Il 
suit que 
\begin{align}
        I(Z; X) &= \mathbb{E}_{p(\mathbf{x})}\mathbb{E}_{q_\phi(\mathbf{z} \mid \mathbf{x})} \bigg[\log \frac{q_\phi(\mathbf{x} \mid \mathbf{z})}{p(\mathbf{x})} \bigg]\, ;  \\[1.5ex]
                ...
                &\geq \mathbb{E}_{p(\mathbf{x})}\mathbb{E}_{q_\phi(\mathbf{z} \mid \mathbf{x})} \bigg[\log p_\theta (\mathbf{x} \mid \mathbf{z})\bigg]\, .
\end{align}
À la dernière ligne, on a éliminer l'entropie du message $H(X)$ puisque c'est une constante qui ne dépend pas de $\phi$, et 
...
\begin{equation}\label{eq:beta VAE}
       \mathcal{L}_{\phi,\theta,\beta}(\mathbf{x}) =  
 \mathbb{E}_{q_\phi(\mathbf{z} \mid \mathbf{x})} \bigg[\log p_\theta (\mathbf{x} \mid \mathbf{z})\bigg]
 -
 \beta  \mathbb{E}_{q_\phi(\mathbf{z} \mid \mathbf{x})} \bigg[ 
                ...
                \bigg]
\end{equation} 
On note qu'on a délaisser la constante $\alpha$, principalement dû au fait que le terme de régularisation est une limite inférieur et 
la valeur de $\alpha$ est difficile à déterminer en pratique. Réintégrer la constante $\alpha$ dans la ELBO 
reviendrait à choisir la fonction objective qui est le sujet du travail par \citet{Zhao2017}.

Il est intéressant de noter que cette nouvelle dérivation commence d'un point de vue complètement différent. On 
...
directement le nombre de partitions possibles dans l'espace latent \citep{}
%\begin{align}
