...
est un phénomène particulièrement riche en information\footnote{ 
        ...
        observe que les amas globulaires et les galaxies sont des candidats idéals pour une recherche systématique du 
        phénomène.
        }. 
...
\end{equation}
Par exemple, une galaxie spirale comme la Voie Lactée, $M\sim 10^{12} M_{\odot}$, à une distance caractéristique $D= 2\, \mathrm{Gpc}$ produirait 
$\theta_E \sim 2'' $.
...
\end{equation} 
où $\xi$ est la taille physique d'un objet placé à une certaine distance de l'observateur, et $\theta$ est l'angle sous-tendu 
par cet objet. Pour un Univers plat décrit par la métrique de Friedmann-Robertson-Lemaître-Walker (FRLW),
cette distance s'exprime directement en terme du décalage vers le rouge des photons émis par l'objet observé 
%\begin{equation}\label{eq:diameter angular distance}
        %D = \frac{1}{(1 + z)H_0} \int_{z_2}^{z_1}
%\end{equation} 
%où $\xi$ est une distance comobile, ç.-à-d. constante vis-à-vis l'expansion des coordonnées physiques, et 
...
L'objectif d'entraînement de la fonction $f_\phi$ nécessite de manipuler la ELBO de pour obtenir une 
divergence KL
\begin{align}
