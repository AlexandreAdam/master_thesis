...
%définie dans l'ensemble d'entraînement $\mathcal{D}$. 
...
%Une solution à ce problème est d'ajouter des biais inductifs au problème d'apprentissage. Dans ce cas, 
%il est possible de choisir a priori des algorithmes d'apprentissages qui performent bien pour une classe 
%de problème qui nous intéresse, ce qui contourne le théorème NFL.

Dans le contexte du méta-apprentissage, la généralisation réfère plutôt au concept de transfert d'apprentissage, ç.-à-d. le transfert des connaissances 
...
%Pour un problème inverse, le premier critère à satisfaire 
%est la capacité de l'hypothèse, $\hat{\mathbf{x}}^{(T)}$, à reproduire l'observation $\mathbf{y}$.
%comme le test la de validation croisée \citep{Wolpert1992b} 
%pour estimer la qualité de la généralisation de $f_\theta$ \citep[e.g.][]{}
%ou comme la comparaison de la vraisemblance ou 

%\citep{Schimdhuber1987,Naik1992,bengio1992,Thrun1998}
...
%Les travaux plus récent d'\citet{Andrychowicz2016} montrent aussi que cette approche pour le 
%méta-apprentissage peut s'appliquer dans des situations
...
%\subsection{Note sur les biais inductifs des réseaux convolutifs}
...
% Does not work, or I did not managed to make it work
%Pour éclaircir ce dernier point, on peut considérer le problème d'apprentissage avec comme objectif de recouvrir 
%une fonction $f(x)$ à partir d'un ensemble limité d'exemples. On considère plus spécifiquement 
%une fonction possédant une série de Taylor infinie et un ensemble d'entraînement avec un support fini 
%et un nombre limité de points de données
%\begin{equation}\label{eq:RegressionProblem}
        %\begin{aligned}
                %f_{\theta^{\star}} &= \underset{f_\theta \in \mathcal{H}}{\min}\,\,\mathbb{E}_{(x,y) \sim \mathcal{D}} \bigg[(f_\theta(x) - y)^2\bigg]\,; \\[1.5ex]
                %\mathcal{D} &= \bigg\{\left(x^{(i)},\, \sin(5x^{(i)} + \frac{\pi}{2})\right)\, \bigg| \, x^{(i)} \sim \mathcal{U}(-2, 2)\bigg\}_{i=1}^{100}$\, .
        %\end{aligned}
%\end{equation} 
%On construit un espace d'hypothèses, $\mathcal{H}$, très peu informatif sur la solution en supposant que $f(x)$ peut être 
%approximé par $f_\theta$, un perceptron \citep{Rosenblatt1958} avec 2 couches cachées et une activation non-linéaire ELU \citep{Clevert2016}. 
%En utilisant un algorithme générique d'optimisation comme ADAM \citep{Kingma2014}, on est en mesure d'ajuster parfaitement 
%l'ensemble d'entraînement, mais notre hypothèse ne se généralise pas au-delà du support fini de $\mathcal{D}$ (voir la figure \ref{fig:regression}).
%\begin{figure}[thb!]
        %\centering
        %\begin{subfigure}[t]{0.3\linewidth}
                %\centering
                %\includegraphics[width=\textwidth]{figures/regression_exemple}
                %\caption{Extrapolation d'un modèle très expressif $f_{\theta}$ ajusté sans biais inductifs avec 
                %$\mathcal{D}$. }
        ...
        %\begin{subfigure}[t]{0.3\linewidth}
                %\centering
                %\includegraphics[width=\textwidth]{figures/reg_learning_curves}
                %\caption{Effet des biais inductifs sur l'interpolation. On compare un biais explicite, soit l'ajout en entrée de $f_\theta$ 
                %des puissances de $x$ pour accélérer l'apprentissage d'une série de Taylor, avec des biais implicite, appris par méta-apprentissage (courbe rouge 
                %pointillée).}
        ...
        %\begin{subfigure}[t]{0.3\linewidth}
                %\centering
                %\includegraphics[width=\textwidth]{figures/regression_exemple}
                %\caption{Extrapolation d'un modèle très expressif $f_\theta$ ajusté avec
                %des biais inductifs encodé dans un modèle $g_{\varphi}: \mathcal{D} \rightarrow \mathcal{H}$.}
                %\label{fig:meta regression}
        %\end{subfigure}
        %\caption{Effet des biais inductifs sur l'apprentissage d'une fonction $f_\theta$.}
        %\label{fig:meta}
%\end{figure}

%La solution à ce problème est particulièrement claire: on doit restreindre l'espace des 
%hypothèses tel que la solution universelle au problème, $f(x)$, peut être retrouvée aisément. 
%L'objectif du méta-apprentissage se concentre précisément sur l'apprentissage de ces biais inductifs, 
%non-seulement pour accélérer le problème d'apprentissage \eqref{eq:RegressionProblem} (voir figure \ref{fig:learning curves}), mais aussi 
%pour résoudre le problème de généralisation, ç.-à-d. réduire le risque empirique d'appliquer 
%l'hypothèse $f_{\theta^\star}$ sur un ensemble de données test, $\mathcal{D}^{\mathrm{test}}$.
%Ce problème ne peut pas être résolue autrement qu'en supposant dès le départ un espace 
%d'hypothèse $\mathcal{H}$ extrêmement informatif sur la solution.


%Une possible 
%résolution de ce problème est l'utilisation de biais inductifs, 
%ç.-à-d. une sélection a priori des hypothèse $f_\varphi \in \mathcal{H}$ possibles. 
%sont strictement nécessaire pour l'apprentissage de fonction universelles \citep{Mitchell1980}. 
%, qui stipule qu'il 
%n'existe pas d'algorithme d'optimisation universelle purement informé par les données d'entraînements. 
...
%pour construire un algorithme d'optimisation, aussi basé sur les cellules LSTM, qui performe 
