...
la notion de distance qui respecte \eqref{eq:diameter angular distance} est la distance du diamètre angulaire $D$. 
...
Pour un Univers plat \citep[voir les manuels de référence][]{Bartelmann2004,Coles2002,Dodelson2003}, on a donc
\begin{align}
        D &= c a(z) \underbrace{\int_{a(z)}^{a(0)} \frac{da}{a \dot{a}}}_{\text{distance comobile}}; \\[2ex]
                \nonumber
...
\end{align}
On a utiliser la relation entre le facteur d'échelle, $a$, et le décalage vers le rouge, $a = (1 + z)^{-1}$, pour obtenir l'équation \eqref{eq:dang} 
...
%\end{equation} 
$\Omega_{r,0}$, $\Omega_{m,0}$ et $\Omega_{\Lambda, 0}$ sont les paramètres de densités, au temps présent, de la radiation, de la matière et l'énergie sombre 
respectivement. 
...
je rapporte la valeur des paramètres du modèle cosmologique $\Lambda$CDM dans l'annexe 
\ref{app:lcdm}, obtenue par l'équipe \citet{PlanckCollaboration2018} qui a analysé 
le fond diffus cosmologique avec l'observatoire spatial Planck.
% Mettre en appendice
%\begin{table}[H]
        %\centering
...
pour inférer approximativement les variables latentes (ou cachées) qui contrôlent un processus génératif 
définie via une distribution marginale implicite comme un échantillon de données. Dans cette section, j'introduis 
...
Pour contourner ce problème, on introduit une distribution variationnelle, $q_\phi(\mathbf{z} \mid \mathbf{x})$, dont le rôle est de 
...
\end{equation}
On est libre de faire ce choix sans pour autant limiter sévèrement les formes possibles de la distribution qui nous intéresse $p_\theta(\mathbf{x})$.
