...
Les paramètres $\alpha$ et $\theta$ sont absorbés dans les paramètres d'inférence $\varphi^{(t)}$, ce qui nous donne 
...
Toutefois, la troisième condition d'Hadamard \ref{hadamard:3} est respectée seulement si $g_{\varphi^{(t)}}(\cdot,\, \cdot)$ 
...
AdaGrad \citep{Duchi2011}, RMSProp\footnote{L'algorithme apparaît dans le cours CSC321 à l'Université de Toronto, donné par Geoffrey Hinton en 2011.} \citep{Hinton2012} 
...
Ainsi, il est important de considérer une classe de modèle avec une certaine mémoire des itérations précédente. Pour ce 
...
qui agissent comme une mémoire des activations précédentes ($0 < t' < t$) du réseau de neuronnes.
Pour un réseau convolutionnel, cette unité est représentée par les opérations suivante:
\begin{align}
...
\end{align}
où $S(x) = \frac{1}{1 + e^{-x}}$ est une fonction sigmoïde et $\mathbf{x}$ est un état d'activation interne au réseau de neuronnes. 
...
%il est utilisé directement dans  
%la RIM du chapitre \ref{chap:censai} pour prétraiter le gradient de la vraisemblance, 
%de façon similaire aux travaux de \citet{Modi2021}.


\subsection{Méta-apprentissage et transfert de l'apprentissage}
