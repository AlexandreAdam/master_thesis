...
\citet{Zwicky1937b} a estimée la masse de l'amas de Coma à $\gtrsim 4.5\times  10^{13}M_\odot$ avec le théorème du viriel. 
Cette limite inférieur est un très bon estimé de 
la valeur acceptée aujourd'hui, dérivé avec les effets de lentilles faibles produites par l'amas 
...
improbables, pointant vers les limitations instrumentales de l'époque. En effet, les télescopes terrestres étaient 
...
et des angles de déflexion qui causerait l'apparition d'une image double du quasar \citep{Young1981,Falco1991}
\begin{figure}[tb!]
        \centering
...
c.-à-d. lentillé par une ou plusieurs galaxies \citep[e.g.][]{Kelly2015,Goobar2017}, et la surveillance décennale de quasars lentillés 
...
De ce fait, beaucoup d'effort a été déployé pour trouver systématiquement ces objets dans le ciel. Le programme 
...
basés sur la recherche visuelle d'images doubles, triples, d'arcs ou d'anneau \citep[e.g.][]{Faure2008} dans les champs larges et profonds comme 
COSMOS \citep{Koekemoer2007,Scoville2007}, connais aujourd'hui une renaissance nourrie par les succès récents de l'apprentissage profond 
...
de l'ordre du kpc; soit 6 ordres de grandeur de différences.
...
utilisé pour représenter $X$ et l'identité $Y=i$ de chaque exemple d'un ensemble de données qu'on veut modéliser $\mathcal{D} = \{\mathbf{x}_i \}_{i=1}^{N}$.
...
\end{align}
À la dernière ligne, on a éliminé l'entropie du message $H(X)$ puisque c'est une constante strictement positive qui ne dépend pas de $\phi$, et 
...
suivant des motivations complètement différentes de celles qu'on a suivies ici. La dérivation accomplit ici 
...
La dérivation accomplit dans cette sous-section commence d'un point de vue complètement différent 
...
ce modèle repose, puis j'introduis l'algorithme d'inférence et les concepts d'apprentissage machinent qui motivent 
...
\end{equation} 
où $\mathbf{y}\in \mathcal{Y}$ est un vecteur d'observables (comme l'image capturé par les capteurs photographiques CCD dans un télescope), 
...
est de haute qualité, ce qui révèle la complexité cachée de ces systèmes \citep[e.g.][]{Schuldt2019}. 
...
l'étude de méthodes robustes d'entraînement des réseaux neuronaux de manière pour prévenir ou défendre contre 
...
Pour un problème de méta-apprentissage, l'ensemble de données d'entraînement est légèrement différent d'une tâche d'interpolation ou de classification. 
...
qui minimisation de l'erreur quadratique moyenne, soit le risque empirique sur l'ensemble d'entraînement
...
à généraliser au-delà d'un ensemble de mesures $\mathcal{D}$ est cruciale. 
Par exemple, un test important de la relativité générale, testée et découverte dans le régime de champs faible, est 
...
informatif sur la nature du problème ou du phénomène d'intérêt (p. ex. la gravité), 
...
un algorithme d'entraînement pour un réseau de neurones avec un taux de convergence plus rapide que les algorithmes d'optimisations génériques.
...
C'est par cette fonction qu'on peut intégrer nos connaissances sur la nature du phénomène à l'étude, p. ex. les équations \eqref{eq:alpha adim} et \eqref{eq:lens equation adim} 
...
Le chapitre \ref{chap:intro} est une introduction détaillée aux méthodes requise pour construire 
cet algorithme, soit le modèle physique pour simuler les chemins lumineux des lentilles gravitationnelles (section \ref{sec:lentilles gravitationnelles}), 
les auto-encodeurs variationnels pour modéliser la distribution implicite des images de galaxies (section \ref{sec:vae}) et 
les machines à inférences récurentielle pour accomplir la reconstruction libre de lentilles gravitationnelles (section \ref{sec:intro rim}).
